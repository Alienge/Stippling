...
epswriter::epswriter(std::string fileName, double minX, double minY, double maxX, double maxY, double flag) : name(fileName), currRed(0), currGreen(0), currBlue(0), currK(0) {
	...
	*eps << "0 0 0 0 setcmykcolor" << std::endl << std::endl;
	*eps << lineWidth << " setlinewidth " << std::endl;
	*eps << "/Helvetica " << fontSize << " selectfont" << std::endl;
...
bool epswriter::equalCurrColor(int r, int g, int b, int k) {
	bool t = true;
	t &= (r == currRed);
	t &= (g == currGreen);
	t &= (b == currBlue);
...
void epswriter::changeColor(int r, int g, int b, int k) {
	if (!equalCurrColor(r, g, b, k)) {
		*eps << r / 65535.0 << " " << g / 65535.0 << " " << b / 65535.0 << " " << k / 65535.0 << " setcmykcolor" << std::endl;
		currRed = r; currGreen = g; currBlue = b; currK = k;
	}
}
...
void epswriter::drawBasicCircle(double xcentre, double ycentre, double radius, int red, int green, int blue, int k) {

	changeColor(red, green, blue, k);
	*eps << xcentre << " " << ycentre << " moveto" << std::endl;
	*eps << xcentre << " " << ycentre << " " << radius << " 0 360 arc" << std::endl;
	*eps << "closepath" << std::endl;
}
...
void epswriter::filledCircle(double xcentre, double ycentre, double radius, int red, int green, int blue, int k)
{
	drawBasicCircle(xcentre, ycentre, radius, red, green, blue, k);
	*eps << "fill" << std::endl;
}
